...
public class FreeGuideCmdArgs {
	...
	public FreeGuideCmdArgs(String[] args, Vector booleanFlags) {
		...
		for(int i=0;i<args.length;i++) {
			...
			} else if(arg.startsWith("-")) {// single-character flag(s)
			...
			} else if(foundFlag){	// some data for a flag
			...
			} else {	// An argument without an associated flag
	 ...
	 * @returns the data for a flag or null if the flag was omitted
	 *			or had no data associated with it
	 */
	public String getValue(String name) {
		
		int i = names.indexOf(name);
		
		if(i==-1) {	// If this flag was omitted
	 ...
	 *
	 * @param name the String name of the flag required
	 * @returns true if the flag was used at the command
	...
	/**
	 * Provides all the command line arguments which were not
	 * given as data to a flag.
	 *
	 * @returns the "blank" command line arguments not associated
	 *          with a flag
	 */
	 ...
	 * Takes a char argument and adds it as a String.
	 *
	...
	private Vector names;		// The names of flags passed as arguments
	private Vector values;		// The values of arguments to each flag
	private Vector blankValues;	// The arguments not associated with any flag
