...
#
# Adapted to allow writing to a file by Andy Balaam 2002-01-04
...
# Added to allow writing to a file by Andy Balaam 2002-01-04:
use IO;
...
# write_data()
#
# Write a complete XMLTV file to stdout or a file if given.
# Adapted to allow writing to a file by Andy Balaam 2002-01-04
#
# Parameters: listref of four elements as returned by read_data(), followed
# by a filename if required.
#
sub write_data( $ ) {
    my $data = shift;
    my $writer;

	# Addition by Andy Balaam 2002-01-04:
	# If a filename is given, pass it as the output
	if($data->[4]){
		
		my $tmp = $data->[4];
		my $output = new IO::File(">$tmp");
		
		$writer = new XMLTV::Writer(encoding => $data->[0],OUTPUT => $output);
	} else {
		$writer = new XMLTV::Writer(encoding => $data->[0]);
	}
...
sub new {
    ...
    # Removed by Andy Balaam since location of dtd is unknown
	# and this causes problems with a validating parser.
	#{
