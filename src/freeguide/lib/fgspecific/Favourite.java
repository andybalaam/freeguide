...
public class Favourite
{
    ...
    public boolean matches( Programme prog )
    {
        ...
        if( 
            ( titleRegex != null )
                && !titleRegex.matcher( progTitle ).matches(  ) )
        {
        ...
        if( 
            ( dayOfWeek != null )
                && ( dayOfWeek.intValue(  ) != prog.getStart(  ).get( 
    ...
    /**
     * Returns the ID of the channel for this favourite, or an empty string if
     * the channel is null.
     *
     ...
     *
     * @param channel The new channelID value
     */
