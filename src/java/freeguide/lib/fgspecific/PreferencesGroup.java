...
public class PreferencesGroup
{
    ...
    private final static SimpleDateFormat chosenDateFormat =
     ...
     *
     * @return true unless errors were encountered in the course of
    ...
    /**
     * Gets the offsetForDate attribute of the FreeGuidePreferencesGroup
     * object
     *
     ...
     * @return Description of the Return Value
     */
    public String performSubstitutions( 
        String in, boolean preserveDoublePercents )
    {

        return performSubstitutions( 
            in, GregorianCalendar.getInstance(  ), preserveDoublePercents );
     ...
     */
    public String performSubstitutions( 
        String in, Calendar date, boolean preserveDoublePercents )
    {
            ...
            else
            {
                ...
                if( thePref == null )
                {
                    ...
                    else if( theKey.toLowerCase(  ).equals( "date" ) )
                    {
                        ...
                        //TODO!!! sub = ViewerFrame.fileDateFormat.format( date.getTime(  ) );
                    }
        ...
        try
        {
            ...
            for( int i = 0; i < keys.length; i++ )
            {
                ...
                if( keys[i].startsWith( "day-" ) )
                {
                    ...
                    {
                        cal.setTime( 
                            chosenDateFormat.parse( keys[i].substring( 4 ) ) );
    ...
    public Vector getInGuideProgrammes( Calendar date )
    {
        ...
        TVProgramme prog =
            chosen_progs.getProgramme( String.valueOf( i ), null );
    ...
    public int[] getInGuideProgrammeKeys( Calendar date )
    {
        ...
        if( anythingInGuide( date ) )
        {
            ...
            //    FreeGuide.prefs.misc.getTime( "day_start_time" );
            while( prog != null )
            {
                progStart = GregorianCalendar.getInstance(  );

                progStart.setTimeInMillis( prog.getStart(  ) );
                ...
                if( 
                    ( progStart.get( Calendar.DAY_OF_YEAR ) == date.get( 
                            Calendar.DAY_OF_YEAR ) )
                        && ( progStart.get( Calendar.YEAR ) == date.get( 
                            Calendar.YEAR ) ) )
                {
     ...
     */
    public void addInGuide( TVProgramme prog, Calendar date )
    {
     ...
     */
    public void removeFromGuide( TVProgramme prog )
    {
    ...
    public List getFavourites(  )
    {
        ...
        while( 
            ( fav = favourites.getFavourite( String.valueOf( i ), null ) ) != null )
        {
            ans.add( fav );
    ...
    public ChannelSet[] getChannelSets(  )
    {
        ...
        ChannelSet cset =
            channelsets.getChannelSet( String.valueOf( i ), null );
    ...
    /**
     * Given a line cat.key=value, returns an FGPreferences object
     * representing the category, a string for the key and a string for the
     * value.
     *
    ...
    private Vector processPrefLine( String line )
    {
        ...
        if( i == -1 )
        {
            FreeGuide.die( 
                FreeGuide.msg.getString( 
                    "invalid_preference_string_applied_no_dot" ) );
        ...
        else
        {
            ...
            FreeGuide.die( 
                FreeGuide.msg.getLocalizedMessage( 
                    "unknown_preferences_group_template", messageArguments ) );
    ...
    /**
     * Takes a line in a form like this: misc.install_directory=C:\Program
     * Files\FreeGuide and translates that to place the relevant entry into
     * the correct user preferences node..
     *
