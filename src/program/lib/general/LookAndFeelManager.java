...
public class LookAndFeelManager {
	...
	/**
	 * Place to store name/classname of available L&F's so we only have
	 * to check availability the hard way once.
	 */
	...
	private static void createMap() {
		...
		for (int i = 0; i < laf.length; i++) {
			...
			if (isAvailable(className)) {
				availableLAFs.put(laf[i].getName(),
							   className);
			}
		}
