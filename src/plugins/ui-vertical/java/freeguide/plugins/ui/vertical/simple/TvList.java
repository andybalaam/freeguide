...
public class TvList extends JTable implements MouseListener
{
    ...
    {
        int nColumn = this.getTableHeader().getColumnModel().getColumnIndexAtX(mouseEvent.getX());
        switch(nColumn) {
            case TvTableModel.COL_CHANNEL:
                this.mnuChannels.show(mouseEvent.getComponent(), mouseEvent.getX(), mouseEvent.getY());
                break;
            case TvTableModel.COL_TITLE:
                if (this.mnuTitle == null) {
                    this.mnuTitle = new GenericFilterMenu(this.titleFilter, TitleDialog.class);
                }
                this.mnuTitle.show(mouseEvent.getComponent(), mouseEvent.getX(), mouseEvent.getY());
                break;
            case TvTableModel.COL_TIME:
                if (this.mnuTime == null) {
                    this.mnuTime = new GenericFilterMenu(timeFilter, TimeDialog.class);
                }
                this.mnuTime.show(mouseEvent.getComponent(), mouseEvent.getX(), mouseEvent.getY());
                break;
        }
    }//public void mouseClicked(MouseEvent mouseEvent)
