...
{
    /**
     * The time at which the program has to start (or later)
     * -1 means that this field is not used for filtering.
     * If it's not -1, the meaning is "HHmm" as integer (2-digit hour
     * with 24 hour format)
     */
    protected int nStartTime   = -1;

    /**
     * The time at which the program may start at last
     * -1 deactivates that field, normal format is "HHmm"
     */
    ...
    public boolean showProgramme(TVProgramme programme)
    {
        ...
        int nTime = Integer.parseInt(time.format(new Date(programme.getStart())));
    ...
    protected void setTitle()
    {
        if (this.nStartTime == -1 && this.nEndTime == -1) {
            this.setTitle(VerticalViewer.getInstance().getLocalizer().getLocalizedMessage("timefilter.anytime"));
        } else {
            StringBuffer strTitle = new StringBuffer(20);
            strTitle.append(VerticalViewer.getInstance().getLocalizedMessage("timefilter.begins")).append(" ");
            if (this.nStartTime != -1) {
                strTitle.append(getTimeFromInt(this.nStartTime));
                if (this.nEndTime != -1) {
                    strTitle.append(" - ").append(getTimeFromInt(this.nEndTime));
                }
            } else {
                strTitle.append(VerticalViewer.getInstance().getLocalizedMessage("timefilter.until")).append(" ").append(getTimeFromInt(this.nEndTime));
            }
