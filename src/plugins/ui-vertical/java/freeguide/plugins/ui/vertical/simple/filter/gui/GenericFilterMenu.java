...
public class GenericFilterMenu extends JPopupMenu implements ActionListener
{
    ...
    {
        this.mnuEdit = new JMenuItem(VerticalViewer.getInstance().getLocalizedMessage("genericfilter.menu.editfilter"));
        this.mnuEdit.addActionListener(this);
        this.add(this.mnuEdit);

        this.mnuNoFilter = new JMenuItem(VerticalViewer.getInstance().getLocalizedMessage("genericfilter.menu.nofilter"));
    ...
    public void actionPerformed(ActionEvent e)
    {
        if (e.getSource() == this.mnuEdit) {
            ...
            this.filter.importSettings((String)((DataMenuItem)e.getSource()).getData());
        }
    }//public void actionPerformed(ActionEvent e)
    ...
    /**
     * Shows the time setting dialog and calls the addRecent()
     * method if the dialog has been closed with Ok
     */
    ...
    protected void addRecent()
    {
        ...
        for (int nA = 0; nA < arItems.length; nA++) {
            if (((DataMenuItem)arItems[nA]).getData().equals(strExportValue)) {
                bExists = true;
                break;
            }
        }
        ...
        if (!bExists) {
            JMenuItem item = new DataMenuItem(this.filter.getTitle(), strExportValue);
