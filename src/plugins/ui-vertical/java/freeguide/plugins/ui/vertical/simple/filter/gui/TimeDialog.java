...
/**
 * Dialog in which the user can configure a TimeFilter.
 * The layout has been copied shamelessly from nextVEPG.
 *
 * @author Christian Weiske <cweiske@cweiske.de>
 */
public class TimeDialog extends JDialog implements AdjustmentListener, ActionListener, SettingDialog
{
    /**
    ...
    protected void buildLayout()
    {
        ...
        this.setTitle(VerticalViewer.getInstance().getLocalizedMessage("timedialog.title"));
        ...
        this.btnOk = new JButton(VerticalViewer.getInstance().getLocalizedMessage("timedialog.ok"));
        this.btnOk.setDefaultCapable(true);
        this.btnOk.addActionListener(this);
        this.btnCancel = new JButton(VerticalViewer.getInstance().getLocalizedMessage("timedialog.cancel"));
        ...
        this.chkUseStart = new JCheckBox(VerticalViewer.getInstance().getLocalizedMessage("timedialog.check.from"));
        ...
        this.chkUseEnd = new JCheckBox(VerticalViewer.getInstance().getLocalizedMessage("timedialog.check.to"));
    ...
    /**
     * Loads the settings that the filter already has
     * into the GUI
     */
    ...
    public void adjustmentValueChanged(AdjustmentEvent adjustmentEvent)
    {
        if (adjustmentEvent.getSource() == this.scrStart) {
            ...
            this.lblStartValue.setText(TimeFilter.getTimeFromInt(getTimeIntFromScrollInt(nStart)));
            this.chkUseStart.setSelected(true);
            if (this.chkUseEnd.isSelected() && nStart > this.scrEnd.getValue()) {
                this.scrEnd.setValue(nStart);
            }
        } else {
            int nEnd = this.scrEnd.getValue();
            this.lblEndValue.setText(TimeFilter.getTimeFromInt(getTimeIntFromScrollInt(nEnd)));
            this.chkUseEnd.setSelected(true);
            if (this.chkUseStart.isSelected() && nEnd < this.scrStart.getValue()) {
    ...
    /**
     * Converts a scroll bar integer to a "time integer"
     * Scrollbars use a step of 1 to increment 5 minutes,
     * and this methods converts from the scrollbar value
     * to a TimeFilter value of e.g. "2015" for "20:15"
     */
    ...
    public void actionPerformed(ActionEvent actionEvent)
    {
        if (actionEvent.getSource() == this.btnOk) {
            ...
            if (this.chkUseStart.isSelected()) {
                this.timeFilter.setStartTime(getTimeIntFromScrollInt(this.scrStart.getValue()));
            } else {
                this.timeFilter.setStartTime(-1, false);
            }
            if (this.chkUseEnd.isSelected()) {
                this.timeFilter.setEndTime(getTimeIntFromScrollInt(this.scrEnd.getValue()));
            } else {
